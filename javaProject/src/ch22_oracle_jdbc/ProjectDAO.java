...
public class ProjectDAO {
	...
	public Vector list() {
		...
		try {
			conn=DB.dbConn();
			String sql="SELECT student_no,name,attendance,late,early,go_out,absence,floor(absence+(late+early+go_out)/3.0) sum FROM attendlist";
	...
	public int update(ProjectDTO dto) {
		...
		try {
			conn=DB.dbConn();
			String sql="update attendlist set name=?,attendance=?,late=?,early=?,go_out=?,absence=? where student_no=?";
			pstmt=conn.prepareStatement(sql);
	...
	public int delete(String student_no) {
		...
		try {
			conn=DB.dbConn();
			String sql="delete from attendlist where student_no=?";
			pstmt=conn.prepareStatement(sql);
	...
	public ProjectDTO view(String student_no) {
		...
		try {
			conn=DB.dbConn();
			String sql="SELECT student_no,name,attendance,late,early,go_out,absence,floor(absence+(late+early+go_out)/3.0)absence FROM attendlist where student_no=?";
			pstmt=conn.prepareStatement(sql);
			pstmt.setString(1, student_no);
			rs=pstmt.executeQuery();
			if(rs.next()){
	...
	public Vector per() {
		...
		try {
			conn=DB.dbConn();
			String sql="SELECT student_no,name,attendance,floor(absence+(late+early+go_out)/3.0) SUM, ROUND(((attendance+floor(absence+(late+early+go_out)/3.0))/180)*100, 2) a, round((attendance/180)*100,2) b FROM attendlist";
			pstmt=conn.prepareStatement(sql);
			rs=pstmt.executeQuery();
			while(rs.next()) {
				Vector rows=new Vector();
